        ...
        <div class="mt-6 space-y-6">
            <div class="card card-border bg-base-200/20">
                ...
                <div class="grid grid-cols-2 gap-4 p-4 sm:gap-8 sm:p-6 lg:grid-cols-4">
                    <div
                        v-for="(shadow, index) in shadowVariants"
                        :key="index"
                        class="flex items-center justify-center gap-2">
                        <div
                            :class="`bg-base-100 rounded-box text-base-content/60 flex size-36 items-center justify-center text-sm md:size-48 ${shadow.class}`">
                            {{ shadow.name }}
                        </div>
                    </div>
                </div>
            ...
            <div class="card card-border">
                ...
                <div class="grid grid-cols-2 gap-4 p-4 sm:gap-8 sm:p-6 lg:grid-cols-4">
                    <div
                        v-for="(shadow, index) in insetShadowVariants"
                        :key="index"
                        class="flex items-center justify-center gap-2">
                        <div
                            :class="`bg-base-100 border-base-200 rounded-box text-base-content/60 flex size-36 items-center justify-center border p-3 text-center text-xs sm:text-sm md:size-48 ${shadow.class}`">
                            {{ shadow.name }}
                        </div>
                    </div>
                </div>
            ...
            <div class="card card-border">
                ...
                <div class="flex flex-col gap-3 p-4 sm:p-6">
                    <p
                        v-for="(shadow, index) in textShadowVariants"
                        :key="index"
                        :class="`font-semibold capitalize sm:text-lg md:text-xl ${shadow.class}`">
                        {{ shadow.name }}
                    </p>
                </div>
            </div>
