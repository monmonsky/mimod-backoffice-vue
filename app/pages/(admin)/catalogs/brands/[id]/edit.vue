...
const { data: brandResponse, pending } = await useAsyncData(`brand-${brandId}`, () => {
...
const handleSubmit = async () => {
    try {
        ...
        console.error("=== ERROR UPDATE BRAND ===");
        console.error("Error:", err);
        console.error("Error Data:", err?.data);
        console.error("Error Message:", err?.data?.message);
        console.error("========================");

        const errorMessage = getErrorMessage(err, "Failed to update brand");
...
<template>
    <div>
        <PageTitle
            :title="'Edit Brand'"
            :items="[{ label: 'Catalogs' }, { label: 'Brands' }, { label: 'Edit', active: true }]" />
