<script setup lang="ts">
import PageTitle from "~/components/PageTitle.vue";
import OrdersTable from "../OrdersTable.vue";

definePageMeta({
    layout: "admin",
    middleware: "auth",
});

const { setPageTitleFromMenu } = useAdminMenu();
setPageTitleFromMenu();

// Pre-filter for completed orders
const initialFilters = {
    status: 'completed'
};
</script>

<template>
    <div>
        <PageTitle :title="'Completed Orders'" :items="[{ label: 'Orders' }, { label: 'Completed', active: true }]" />

        <!-- Alert Info -->
        <div class="alert alert-success shadow-sm mt-6 mx-6">
            <span class="iconify lucide--check-circle size-5" />
            <span>Showing only orders with <strong>completed</strong> status. These orders have been successfully delivered.</span>
        </div>

        <!-- Reuse OrdersTable with initial filters -->
        <div class="mt-6">
            <OrdersTable :initial-filters="initialFilters" :locked-filters="['status']" />
        </div>
    </div>
</template>
