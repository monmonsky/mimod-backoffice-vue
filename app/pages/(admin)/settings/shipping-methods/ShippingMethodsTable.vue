...
import { getShippingTypeLabel, getShippingTypeBadgeClass, getShippingProviderBadgeClass } from "~/utils/badgeHelpers";
import ShippingMethodsFilterDrawer from "./components/ShippingMethodsFilterDrawer.vue";

const { deleteShippingMethod } = useShippingMethods();
const { success, error: showError } = useToast();

// Permission checks
const { canCreate, canUpdate, canDelete, canShow } = usePermissionCheck();

// Store the shipping method object for displaying in modal
const selectedMethod = ref<any>(null);

// Delete confirmation composable
const {
    confirmDelete,
    itemToDelete: shippingMethodIdToDelete,
...
const { data: shippingMethodsResponse, pending, refresh, error: fetchError } = await useAsyncData(
    "shipping-methods",
    () =>
        $fetch("/shipping-methods", {
...
const shippingMethods = computed(() => {
    const response = shippingMethodsResponse.value as any;
        ...
        <div class="card bg-base-100 shadow">
            <div class="card-body p-0">
                ...
                <div class="flex items-center justify-between px-5 pt-5">
                    ...
                    <div class="inline-flex items-center gap-3">
                        <NuxtLink
                            v-if="canCreate('shipping-methods')"
                            to="/settings/shipping-methods/create"
                            aria-label="Create shipping method"
                            class="btn btn-primary btn-sm max-sm:btn-square">
                            <span class="iconify lucide--plus size-4" />
                            <span class="hidden sm:inline">New Shipping Method</span>
                        </NuxtLink>
                        <label
                            for="shipping-methods-filter-drawer"
                ...
                <div class="mt-4 overflow-auto">
                    ...
                    <!-- Empty State -->
                    <div v-else-if="shippingMethods.length === 0" class="flex flex-col items-center justify-center py-12">
                        <span class="iconify lucide--truck text-base-content/30 mb-4 size-16" />
                        <p class="text-base-content/60">No shipping methods found</p>
                    </div>
                    ...
                    <table v-else class="table">
                        ...
                        <tbody>
                            <tr v-for="method in shippingMethods" :key="method.id">
                                ...
                                <td>
                                    <span :class="['badge badge-sm', getShippingTypeBadgeClass(method.type)]">
                                        {{ getShippingTypeLabel(method.type) }}
                                    </span>
                                </td>
                                <td>
                                    <span :class="['badge badge-sm badge-outline', getShippingProviderBadgeClass(method.provider)]">
                                        {{ method.provider }}
                                    </span>
                                </td>
                                <td class="text-sm">
                                    <div v-if="method.base_cost > 0">
                                ...
                                <td>
                                    <div class="flex justify-end gap-2">
                                        <NuxtLink
                                            v-if="canShow('shipping-methods')"
                                            :to="`/settings/shipping-methods/${method.id}/show`"
                                            class="btn btn-ghost btn-xs"
                                            aria-label="View">
                                            <span class="iconify lucide--eye size-4" />
                                        </NuxtLink>
                                        <NuxtLink
                                            v-if="canUpdate('shipping-methods')"
                                            :to="`/settings/shipping-methods/${method.id}/edit`"
                                            class="btn btn-ghost btn-xs"
                                            aria-label="Edit">
                                            <span class="iconify lucide--pencil size-4" />
                                        </NuxtLink>
                                        <button
                                            v-if="canDelete('shipping-methods')"
