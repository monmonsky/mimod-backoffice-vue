...
// Fetch shipping config detail
const { data: configResponse, pending } = await useAsyncData(
    `shipping-config-detail-${configId}`,
    () => {
        return $fetch(`/shipping-method-configs/${configId}`, {
...
<template>
    <div>
        <PageTitle
            :title="config?.name || 'Shipping Config Details'"
            :items="[
                { label: 'Settings' },
                { label: 'Shipping' },
                { label: 'Shipping Configs', to: '/settings/shipping-configs' },
        ...
        <div v-else-if="config" class="mt-6 space-y-6">
            ...
            <!-- Shipping Methods Using This Config -->
            <div v-if="config.methods && config.methods.length > 0" class="card bg-base-100 shadow">
                <div class="card-body">
                    <h3 class="card-title text-lg">Shipping Methods</h3>
                    <div class="divider my-2"></div>
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <tr v-for="method in config.methods" :key="method.id">
                                    ...
                                    <td>
                                        <span
                                            :class="['badge badge-sm', method.is_active ? 'badge-success' : 'badge-error']">
                ...
                <div class="card-body">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <p class="text-base mt-1">{{ new Date(config.created_at).toLocaleString() }}</p>
                        </div>
                        <div>
                            <label class="text-base-content/60 text-sm font-medium">Updated At</label>
                            <p class="text-base mt-1">{{ new Date(config.updated_at).toLocaleString() }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div v-else class="flex flex-col items-center justify-center py-12">
            ...
            <p class="text-base-content/60 mb-4">Shipping config not found</p>
            <NuxtLink to="/settings/shipping-configs" class="btn btn-primary btn-sm">
