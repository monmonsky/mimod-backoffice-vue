...
const { getPaymentConfig, updatePaymentConfig } = usePaymentConfigs();
...
const { data: configResponse, pending } = await useAsyncData(
    `payment-config-${configId}`,
    async () => {
        return await getPaymentConfig(configId);
    }
);
            ...
            if (data) {
                form.value = {
                    ...
                    configs: Object.entries(data.configs || {}).map(([key, value]) => ({
                        ...
                        is_encrypted: false, // Will be encrypted on server if needed
                    })),
                };
            }
        }
...
const handleSubmit = async () => {
    ...
    try {
        saving.value = true;
        await updatePaymentConfig(configId, form.value);
        success("Payment config updated successfully");
        router.push("/settings/payment-configs");
    } catch (err: any) {
        showError(err?.data?.message || "Failed to update payment config");
    } finally {
        saving.value = false;
    }
};
    ...
    <div>
        <PageTitle
            ...
            :items="[
                { label: 'Settings' },
                { label: 'Payment' },
                { label: 'Payment Configs', path: '/settings/payment-configs' },
        ...
        <form v-else class="mt-6" @submit.prevent="handleSubmit">
            <div class="grid grid-cols-1 gap-5">
                ...
                <div class="card bg-base-100 shadow">
                    <div class="card-body gap-0">
                        ...
                        <p class="text-base-content/60 mb-4 text-sm">Manage key-value pairs for your payment gateway configuration</p>
            ...
            <div class="mt-6 flex justify-end gap-3">
                <NuxtLink class="btn btn-sm btn-ghost" to="/settings/payment-configs">
