...
const { createPaymentConfig } = usePaymentConfigs();
...
// Add first config item on mount
onMounted(() => {
...
const handleSubmit = async () => {
    ...
    try {
        saving.value = true;
        await createPaymentConfig(form.value);
        success("Payment config created successfully");
        router.push("/settings/payment-configs");
    } catch (err: any) {
        showError(err?.data?.message || "Failed to create payment config");
    } finally {
        saving.value = false;
    }
};
            ...
            <div class="grid grid-cols-1 gap-5">
                ...
                <div class="card bg-base-100 shadow">
                    <div class="card-body gap-0">
                        ...
                        <p class="text-base-content/60 mb-4 text-sm">Add key-value pairs for your payment gateway configuration</p>
