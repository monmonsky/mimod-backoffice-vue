<script setup lang="ts">
const authStore = useAuthStore();
const { success } = useToast();

const handleLogout = async () => {
    await authStore.logout();
    success("Logged out successfully");
};
</script>

<template>
    <div>
        <div class="drawer drawer-end">
            <input id="topbar-profile-drawer" type="checkbox" class="drawer-toggle" />
            <div class="drawer-content">
                <label for="topbar-profile-drawer" class="btn btn-ghost max-sm:btn-square gap-2 px-1.5">
                    <div class="avatar">
                        <div class="bg-base-200 mask mask-squircle w-8">
                            <img src="/images/avatars/1.png" alt="Avatar" />
                        </div>
                    </div>
                    <div class="text-start max-sm:hidden">
                        <p class="text-sm/none">{{ authStore.user?.name || 'User' }}</p>
                        <p class="text-base-content/50 mt-0.5 text-xs/none">{{ authStore.user?.role_display_name || 'Member' }}</p>
                    </div>
                </label>
            </div>
            <div class="drawer-side">
                <label for="topbar-profile-drawer" aria-label="close sidebar" class="drawer-overlay" />
                <div class="h-full w-72 p-2 sm:w-84">
                    <div class="bg-base-100 rounded-box relative flex h-full flex-col pt-4 sm:pt-8">
                        <label
                            for="topbar-profile-drawer"
                            class="btn btn-xs btn-circle btn-ghost absolute start-2 top-2"
                            aria-label="Close">
                            <span class="iconify lucide--x size-4" />
                        </label>

                        <div class="flex flex-col items-center">
                            <div class="relative">
                                <div
                                    class="avatar bg-base-200 isolate size-20 cursor-pointer overflow-hidden rounded-full px-1 pt-1 md:size-24">
                                    <img src="/images/avatars/1.png" alt="User Avatar" />
                                </div>
                                <div
                                    class="bg-base-100 absolute end-0 bottom-0 flex items-center justify-center rounded-full p-1.5 shadow-sm">
                                    <span class="iconify lucide--pencil size-4" />
                                </div>
                            </div>

                            <p class="mt-4 text-lg/none font-medium sm:mt-8">{{ authStore.user?.name || 'User' }}</p>
                            <p class="text-base-content/60 mt-1 text-sm">{{ authStore.user?.email || '' }}</p>
                            <div class="badge badge-outline mt-2">{{ authStore.user?.role_display_name || 'Member' }}</div>
                        </div>

                        <div class="border-base-300 mt-4 grow overflow-auto border-t border-dashed px-2 sm:mt-6">
                            <ul class="menu w-full p-2">
                                <li class="menu-title">Quick Links</li>
                                <li>
                                    <NuxtLink to="/dashboards">
                                        <span class="iconify lucide--monitor-dot size-4.5" />
                                        <span>Dashboard</span>
                                    </NuxtLink>
                                </li>
                                <li>
                                    <NuxtLink to="/settings">
                                        <span class="iconify lucide--settings size-4.5" />
                                        <span>Settings</span>
                                    </NuxtLink>
                                </li>
                                <li>
                                    <NuxtLink to="/access-control/user-activity">
                                        <span class="iconify lucide--activity size-4.5" />
                                        <span>Activity Log</span>
                                    </NuxtLink>
                                </li>

                                <li class="menu-title mt-4">Account</li>
                                <li>
                                    <button @click="handleLogout" class="text-error hover:bg-error/10">
                                        <span class="iconify lucide--log-out size-4.5" />
                                        <span>Sign Out</span>
                                    </button>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>
